...
module.exports=async(req,res)=>{try{
 ...
 if(req.method==='POST'){
   ...
   if(action==='request'||action==='call'){
     ...
     const a=(await sbreq(`/rest/v1/astrologers?id=eq.${esc(aid)}&select=*`))[0];if(!a)return json(res,404,{error:'Astrologer profile not found. Please refresh the astrologer list.'});if(!a.verified)return json(res,409,{error:'Astrologer is not approved yet.'});if(!a.online)return json(res,409,{error:'Astrologer is offline. Please ask the astrologer to turn Online ON.'});if(action==='request'&&a.chat_enabled===false)return json(res,409,{error:'Chat is currently disabled by this astrologer.'});if(action==='call'&&a.call_enabled===false)return json(res,409,{error:'Call is currently disabled by this astrologer.'});
