...
module.exports=async(req,res)=>{try{
  ...
  if(req.method==='PATCH'){
    ...
    if(action==='application'){
      const userId=String(b.user_id||''); if(!userId)return json(res,400,{error:'user_id required'});
      const status=String(b.status||''); if(!['approved','rejected'].includes(status))return json(res,400,{error:'status must be approved or rejected'});
      const apps=await sbreq(`/rest/v1/astrologer_applications?user_id=eq.${esc(userId)}&select=*`); const a=apps[0]; if(!a)return json(res,404,{error:'Application not found'});
      if(status==='approved'){
        await sbreq(`/rest/v1/profiles?id=eq.${esc(userId)}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({role:'astrologer',updated_at:new Date().toISOString()})});
        await sbreq('/rest/v1/astrologers',{method:'POST',headers:{Prefer:'resolution=merge-duplicates,return=representation'},body:JSON.stringify({id:userId,bio:a.bio||'',experience_years:a.experience_years||0,expertise:a.expertise||[],languages:a.languages||[],fee:a.requested_fee||0,discount:0,online:false,verified:true,approved_at:new Date().toISOString()})});
      }
      const rows=await sbreq(`/rest/v1/astrologer_applications?id=eq.${esc(a.id)}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({status,admin_note:String(b.admin_note||''),updated_at:new Date().toISOString()})});
      await audit(u.id,'application_'+status,{application_id:a.id,user_id:userId}); return json(res,200,{application:rows[0]||null});
    }
