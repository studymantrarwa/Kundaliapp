...
module.exports=async(req,res)=>{try{
 ...
 if(req.method==='POST'){
   ...
   if(action==='request'||action==='call'){
     ...
     const a=(await sbreq(`/rest/v1/astrologers?id=eq.${esc(aid)}&select=*`))[0];if(!a||!a.verified||!a.online)return json(res,409,{error:'Astrologer is not available right now'});
